...
from src.model.schemas.jwt import JWTAccount, JWToken
...
class JWTGenerator:
    ...
    def _generate_jwt_token(self, *, jwt_data: dict[str, str], expires_delta: datetime.timedelta | None = None) -> str:
    ...
    def generate_access_token(self, account: Account) -> str:
        if not account:
            raise EntityDoesNotExist(f"Не смог сгенерировать токен, так как отсутствует сущность аккаунта")
